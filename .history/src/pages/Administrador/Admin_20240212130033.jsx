...
function Admin() {
  ...
  const [data, setData] = useState([]); // Declarar data aquí
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData(fetchedData); // Actualizar data con la información obtenida del servidor
        ...
        if (storedUser) {
          ...
          const userExists = fetchedData.some(
    ...
    <div className="admin__container">
      ...
      <div className="contenido__derecho">
        ...
        {data.map((usuario, index) => (
          <div key={index} className="usuarios">
            {usuario.username}
          </div>
        ))}
      </div>
