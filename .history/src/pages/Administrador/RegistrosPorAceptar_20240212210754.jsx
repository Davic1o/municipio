...
function RegistrosPorAceptar() {
  ...
  const handleAceptado = async (userId , registrosFiltrados) => {
    try {
      // Encontrar el usuario correspondiente en los datos
      const userToUpdate = registrosFiltrados.find((user) => user._id === userId);
  ...
  const handleRechazado = async (userId) => {
    try {
      ...
      const response = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({ aprobacion: userToUpdate.aproobacion }),
