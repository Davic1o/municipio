...
const useCordActions = () => {
  const [data, setData] = useState([]);

  const handleCordinadorAsignado = async (userId, nuevoID, data) => {
    try {
      // Encontrar el usuario correspondiente en los datos
      const userToUpdate = data.find((user) => user._id === userId);

      // Cambiar el estado del usuario
      userToUpdate.coordinador = nuevoID;
      ...
      const response = await fetch(`/api/users/${userId}`, {
        ...
        body: JSON.stringify({ coordinador: nuevoID }),
      });
  ...
  return { handleCordinadorAsignado, handleAprobacion, handleEstadoN, handleAprobacionN, handleCoord, data, setData };
};

export default useCordActions;
